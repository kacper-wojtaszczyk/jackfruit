...
| Component | Status |
|-----------|--------|
| MinIO local setup (docker-compose) | ✅ Done |
| MinIO client in Go | ✅ Done |
| Object key convention | ✅ Done |
| Auto bucket creation | ✅ Done |
| Immutability contract | ✅ Done |
...
- Extension determined from actual format after unzipping (`.nc`, `.grib2`) — currently hardcoded
...
- [ ] **Retention policy** — keep forever? Archive to Glacier after N days?
- [ ] **Compression** — gzip at ingestion or leave as-is?
- [ ] **S3 versioning** — enable for accidental overwrite protection?
- [ ] **Manifest per run** — JSON sidecar recording inputs, checksums, timing
- [ ] **Content-Type detection** — set proper MIME type on upload
- [ ] **List/query helpers** — Go functions to list by date prefix, filter by dataset
